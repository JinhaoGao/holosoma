...
class OrientationObjectiveTests(unittest.TestCase):
    ...
    def test_first_frame_alignment_matches_initial_robot_frame(self):
        ...
        targets, alignments = self.retargeter._prepare_orientation_targets(
            quaternions,
            self.q,
            num_frames=2,
        )
        robot_matrices, _ = self.retargeter._get_robot_link_orientation_data(
            self.q,
    ...
    def test_world_angular_jacobian_matches_finite_difference(self):
        ...
        reduced_index = int(
            np.flatnonzero(self.retargeter.q_a_indices == qpos_address)[0]
        )
        epsilon = 1e-7
        perturbed_q = self.q.copy()
        perturbed_q[qpos_address] += epsilon
        perturbed_matrices, _ = (
            self.retargeter._get_robot_link_orientation_data(
                perturbed_q,
                self.retargeter.orientation_robot_link_names,
                with_jacobians=False,
            )
    ...
    def test_free_base_angular_jacobian_matches_normalized_qpos_step(self):
        ...
        qx_address = 4
        reduced_index = int(
            np.flatnonzero(retargeter.q_a_indices == qx_address)[0]
        )
        epsilon = 1e-7
        perturbed_q = q.copy()
    ...
    def test_jacobian_step_reduces_geodesic_error(self):
        ...
        angle = 0.02
        target = (
            Rotation.from_rotvec(axis * angle).as_matrix()
            @ current[0]
        ).reshape(1, 3, 3)
        error_before = np.linalg.norm(
            self.retargeter._so3_error_vectors(target, current)
        )
        ...
        error_after = np.linalg.norm(
            self.retargeter._so3_error_vectors(target, updated)
        )
    ...
    def test_quaternion_sign_does_not_change_orientation_matrix(self):
        ...
        matrices = self.retargeter._wxyz_to_matrices(
            np.stack([quaternion, -quaternion])
        )
